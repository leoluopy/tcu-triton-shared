...
static PyObject* launch(PyObject* self, PyObject* args) {{
  ...
  // [TCULauncher-specific]: We don't need the metadata below but just put them
...
static struct PyModuleDef ModuleDef = {{
  ...
  \"__triton_shared_ref_tcu_kernel_launcher\",
  NULL, //documentation
  -1, //size
  ModuleMethods
}};

PyMODINIT_FUNC PyInit___triton_shared_ref_tcu_kernel_launcher(void) {{
...
def compile_module(launcher_src, kernel_placeholder_name):
    ...
    tcu_backend_path = Path(__file__).resolve().parent
    include_dir = os.path.join(tcu_backend_path, "include")
    ...
    def launch(
        ...
        # See TCUUtils.load_binary method.
        ...
        name = "__triton_shared_ref_tcu_kernel_launcher"
...
class TCUUtils(object):
    def __new__(cls):
        if not hasattr(cls, "instance"):
            cls.instance = super(TCUUtils, cls).__new__(cls)
        return cls.instance
    ...
    # and the metadata generated by these functions aren't applicable to the tcu
...
class TCUDriver(DriverBase):

    def __init__(self):
        super().__init__()
        self.utils = TCUUtils()
        self.launcher_cls = TCULauncher
        self.binary_ext = "obj"

    # TCU driver won't be automatically chosen unless explicitly set through
    ...
    def get_current_device(self):
        # TCU doesn't have a device to return. Return something.
        return "tcu"

    def set_current_device(self, device):
        # TCU doesn't have a device to set
        assert device == "tcu"
        return

    def get_current_target(self):
        return GPUTarget("tcu", 0, 0)

    def get_active_torch_device(self):
        import torch
        return torch.device("tcu")
