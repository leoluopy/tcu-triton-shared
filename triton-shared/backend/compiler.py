...
def _ttsharedir_to_llir(ttsharedir: str):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        subprocess.check_call([mlir_opt_path, ttshared_path,
            ...
            # We disable this pass for now since performance on TCU isn't the main
            # focus at the moment.
...
class TCUOptions:
    ...
    shared: bool = False
    # Disable FP8 here since this is a sample TCU backend.
...
class TCUBackend(BaseBackend):
    binary_ext = 'obj'

    @staticmethod
    def supports_target(target: GPUTarget):
        return target.backend == 'tcu'
    ...
    def parse_options(self, opts) -> Any:
        ...
        args.update({k: opts[k] for k in TCUOptions.__dataclass_fields__.keys() if k in opts})
        return TCUOptions(**args)
    ...
    # The TCU backend does not use any extra python modules, return an empty dictionary
